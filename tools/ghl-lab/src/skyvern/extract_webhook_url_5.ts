...
async function main() {
  ...
  try {
      ...
      const result1 = await agent.executeStep(
          "Navigate to the 'Automation' > 'Workflows' page. Wait for the page to load. You will see a list of workflows. Find the workflow named 'WF-04B - AI Call Receiver'. Click on its name to open the builder.",
          URL_WORKFLOWS_LIST
      );
      ...
      const result3 = await agent.executeStep(
          "Look at the settings panel that just opened on the right. There is a section called 'Webhook URL'. Under it, there is a text input box containing a URL that looks like 'https://services.leadconnectorhq.com/hooks/...'. There is also a small copy icon next to it. Extract the exact text from that input box. Return it in your extraction output using this exact JSON schema: {\"webhook_url\": \"the_url_you_copied\"}. Do not click anything else.",
          null
      );
