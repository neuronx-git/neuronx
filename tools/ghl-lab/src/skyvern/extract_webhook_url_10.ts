...
async function main() {
  ...
  try {
      console.log("\nTrying to find the newly created WF-04B and extract the URL using navigation goal...");
      ...
      const result2 = await agent.executeStep(
          "Click on the 'Inbound Webhook' trigger box to open its settings panel on the right side of the screen. Then look at the settings panel, find the 'Webhook URL' input box. The URL should start with 'https://services.leadconnectorhq.com/hooks/'. Select all the text in that input box and copy it. Return the copied text in your extraction output using this exact JSON schema: {\"webhook_url\": \"the_url_you_copied\"}.",
          null
      );
