...
async function main() {
  ...
  try {
      ...
      const result2 = await agent.executeStep(
          "Click on the 'Inbound Webhook' trigger box to open its settings panel on the right side of the screen. Wait for the panel to fully load. In the panel, there is a section called 'Webhook URL'. Under it is a readonly text input containing a URL starting with 'https://services.leadconnectorhq.com/hooks/'. DO NOT click anything. Just read the text value inside that input box. Return the exact text you read in your extraction output using this exact JSON schema: {\"webhook_url\": \"the_url_you_read\"}.",
          null
      );
