...
async function main() {
  ...
  try {
      ...
      const result3 = await agent.executeStep(
          "Look at the settings panel that just opened on the right. There is a section called 'Webhook URL'. Under it, there is a text input box containing a URL. To the right of the input box, there is an icon that looks like two overlapping squares (the copy icon). Click that copy icon exactly. Then, return the URL that you see in the input box in your extraction output using this exact JSON schema: {\"webhook_url\": \"the_url_you_saw\"}.",
          null
      );
