...
async function main() {
  ...
  try {
      console.log("\nTrying to find the newly created WF-04B and extract the URL using copy button...");
      ...
      const result3 = await agent.executeStep(
          "Look at the settings panel that just opened on the right. There is a section called 'Webhook URL'. Next to the URL input box, there is a small copy icon. Click the copy icon. Then return the text you copied in your extraction output using this exact JSON schema: {\"webhook_url\": \"the_url_you_copied\"}.",
          null
      );
      console.log("Step 3 (Extract URL via Copy) Complete:", result3.status);
